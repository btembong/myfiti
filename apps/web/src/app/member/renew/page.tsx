...
type Step = 'identify' | 'select-plan' | 'select-payment' | 'otp-verify' | 'verifying-payment' | 'success' | 'cash-pending'
...
function MemberRenewContent() {
  ...
  const pidParam      = searchParams.get('pid') ?? ''      // Tranzak requestId on return
  const paymentResult = searchParams.get('payment') ?? ''  // 'verify' on return from Tranzak
  ...
  const [result, setResult]                 = useState<{ expires_at?: string; reference?: string; amount?: number; currency?: string; plan_name?: string } | null>(null)
  ...
  const [verifyStatus, setVerifyStatus] = useState<'polling' | 'failed'>('polling')
  ...
  // Auto-lookup member from URL param (coming from email link)
  useEffect(() => {
  ...
  // Handle return from Tranzak — poll payment status
  useEffect(() => {
    if (paymentResult !== 'verify' || !midParam || !gymSlug) return
    setStep('verifying-payment')
    startPolling(pidParam, midParam)
    return () => { if (pollRef.current) clearInterval(pollRef.current) }
  }, [paymentResult, midParam, gymSlug]) // eslint-disable-line react-hooks/exhaustive-deps

  function startPolling(pid: string, mid: string) {
    let attempts = 0
    const MAX = 20 // ~60 s
    pollRef.current = setInterval(async () => {
      attempts++
      try {
        const res  = await fetch(`/api/public/gym/${gymSlug}/payment-status?pid=${encodeURIComponent(pid)}&mid=${encodeURIComponent(mid)}`)
        const data = await res.json() as { status: string; expires_at?: string }
        if (data.status === 'successful') {
          clearInterval(pollRef.current!)
          setResult({ expires_at: data.expires_at ?? undefined })
          setStep('success')
        ...
        if (attempts >= MAX) { clearInterval(pollRef.current!); setVerifyStatus('failed') }
      }
    }, 3000)
  }
  ...
  async function sendOtp() {
    ...
    setOtpError(null)
    try {
      const res  = await fetch(`/api/public/gym/${gymSlug}/send-otp`, {
        method: 'POST',
      ...
      setOtpSentTo(data.sent_to ?? member.email)
      setOtpSent(true)
    } catch {
  ...
  async function verifyOtpAndPay() {
    if (!member || !otpCode.trim()) return
    setVerifyingOtp(true)
    setOtpError(null)
    try {
      const res  = await fetch(`/api/public/gym/${gymSlug}/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ member_id: member.id, otp: otpCode.trim() }),
      })
      const data = await res.json()
      if (!res.ok) { setOtpError(data.error ?? 'Incorrect code.'); return }
      // OTP verified — proceed with wallet payment
      await executeRenew()
    } catch {
  ...
  async function executeRenew() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (paymentMethod === 'wallet') setStep('select-payment')
        setProcessing(false)
        return
      }
      ...
      if (paymentMethod === 'mobile_money') {
        if (!data.payment_url) {
          setRenewError('Payment provider did not return a redirect URL. Please try again.')
          setProcessing(false)
          return
        }
  ...
  const fmt     = (amount: number, currency = 'XAF') => `${currency} ${amount.toLocaleString('fr-CM')}`
  const fmtDate = (iso: string) => new Date(iso).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })

  const subStatusLabel = (s: SubInfo) => {
  ...
  if (gymError)   return <Shell><Card><p style={{ margin: 0, fontSize: 15, color: C.err, textAlign: 'center' }}>{gymError}</p></Card></Shell>
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        <div style={{
          ...
          <a
            ...
            style={{ fontSize: 11, fontWeight: 700, color: C.ok, textDecoration: 'none', letterSpacing: '0.02em' }}
          >
            Open in App →
          </a>
        </div>
          ...
          <Card title="Select a plan" subtitle={`Hi, ${member.name}`}>
            {sub && (
              <div style={{
                ...
                <span style={{ fontSize: 11, fontWeight: 600, color: subStatusColor(sub) }}>{subStatusLabel(sub)}</span>
              </div>
            )}
              ...
              {plans.map(plan => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 800, color: C.pri }}>{fmt(plan.price, plan.currency)}</span>
                  </div>
            ...
            <Btn onClick={() => setStep('select-payment')} disabled={!selectedPlan}>Continue →</Btn>
          </Card>
        )}
        ...
        {/* ── OTP Verification (wallet) ────────────────────────────── */}
        {step === 'otp-verify' && member && (
          <Card title="Verify your identity" subtitle="Wallet payment">
            {!otpSent ? (
              <>
                <p style={{ margin: '0 0 20px', fontSize: 13, color: C.sec, lineHeight: 1.6 }}>
                  We&apos;ll send a 6-digit code to your email address to confirm this payment.
                </p>
                {otpError && <p style={{ margin: '0 0 12px', fontSize: 12, color: C.err }}>{otpError}</p>}
                <div style={{ display: 'flex', gap: 10 }}>
                  <button
                  ...
                  <Btn onClick={sendOtp} loading={sendingOtp} style={{ flex: 2 }}>
                    Send code
                  </Btn>
                </div>
              </>
              ...
              <>
                <p style={{ margin: '0 0 4px', fontSize: 13, color: C.sec, lineHeight: 1.6 }}>
                  Code sent to <span style={{ color: C.pri }}>{otpSentTo}</span>. Enter it below.
                </p>
                <p style={{ margin: '0 0 16px', fontSize: 11, color: C.mut }}>Expires in 5 minutes.</p>
                <input
                  type="text"
                  inputMode="numeric"
                  maxLength={6}
                  ...
                  onKeyDown={e => e.key === 'Enter' && otpCode.length === 6 && verifyOtpAndPay()}
                  placeholder="000000"
                  style={{
                    width: '100%', boxSizing: 'border-box',
                    background: '#111', border: `1px solid ${otpError ? C.err : '#2a2a2a'}`,
                    borderRadius: 8, padding: '10px 14px', color: C.pri,
                    fontSize: 22, fontWeight: 700, letterSpacing: '0.2em',
                    outline: 'none', marginBottom: 8, textAlign: 'center',
                    fontFamily: 'monospace',
                  }}
                />
                {otpError && <p style={{ margin: '0 0 12px', fontSize: 12, color: C.err }}>{otpError}</p>}
                <Btn onClick={verifyOtpAndPay} disabled={otpCode.length !== 6 || verifyingOtp || processing} loading={verifyingOtp || processing}>
                  {processing ? 'Processing payment…' : 'Confirm & pay'}
                </Btn>
                <p
                  onClick={() => { setOtpSent(false); setOtpCode(''); setOtpError(null); sendOtp() }}
                  style={{ margin: '12px 0 0', fontSize: 12, color: C.mut, textAlign: 'center', cursor: 'pointer' }}
                >
                  Resend code
                </p>
              </>
        ...
        {/* ── Verifying Payment (polling after Tranzak redirect) ────── */}
        {step === 'verifying-payment' && (
          <Card title="Verifying payment" subtitle={gym?.name ?? ''}>
            {verifyStatus === 'polling' ? (
              <div style={{ textAlign: 'center', padding: '24px 0' }}>
                <p style={{ margin: '0 0 8px', fontSize: 14, color: C.sec }}>Confirming your payment…</p>
                <p style={{ margin: 0, fontSize: 12, color: C.mut }}>This may take up to a minute.</p>
              </div>
            ) : (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                <p style={{ margin: '0 0 12px', fontSize: 15, color: C.err }}>Payment could not be confirmed.</p>
                <p style={{ margin: '0 0 20px', fontSize: 13, color: C.mut, lineHeight: 1.6 }}>
                  If your money was deducted, it will be refunded. Please try again or contact the gym.
                </p>
                <Btn onClick={() => { setStep('select-plan'); setRenewError(null) }}>Try again</Btn>
              </div>
            )}
          </Card>
        )}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 32 }}>
        ...
        {gym?.name && (
          <span style={{ fontSize: 10, fontWeight: 500, color: C.mut, background: '#111', border: '1px solid #1e1e1e', padding: '2px 8px', borderRadius: 4 }}>
            {gym.name}
          </span>
        )}
      </div>
